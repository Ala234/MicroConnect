...
import {
  ...
  MapPin, Users, Star, Globe, Building2,
} from "lucide-react";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Always fetch base user first
        const userRes  = await fetch(`/api/admin/users/${id}`, { headers });
        ...
        if (userData.role === "influencer") {
          const infRes = await fetch(`/api/admin/users/${id}/influencer-profile`, { headers });
          if (infRes.ok) {
            const infData = await infRes.json();
            setInfluencer(infData);
          }
        } else if (userData.role === "brand") {
          const brandRes = await fetch(`/api/admin/users/${id}/brand-profile`, { headers });
          if (brandRes.ok) {
          ...
          <div className="admin-content">
            ...
            <div className="dashboard-section">
              <div className="profile-hero">
                ...
                {/* Avatar — logo for brand, letter for influencer */}
                {brand?.logo ? (
                  <img
                    src={brand.logo}
            ...
            {/* ACCOUNT DETAILS — shared for both roles */}
            <div className="dashboard-section">
